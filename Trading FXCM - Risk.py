...
def get_day_data(start = start, end = end):
    ...
    tickers_instr_quandl = ['CHRIS/EUREX_FGBL1.4', 'CHRIS/ICE_AR1.4']    
    tickers_factors = ['SPY', 'DBP', 'JJC', 'USO', 'UNG', 'UUP', 'FXY']
    #tickers_factors = ['SPY', 'UUP', 'DBP', 'DBE']
    tickers_vol = ["YAHOO/INDEX_GSPC.4", "CBOE/VIX.4", "CBOE/VXV.4", "CBOE/OVX", "CBOE/GVZ", "CBOE/VXSLV.4", "CBOE/EVZ"]
    
    RawDataInstr = pdr.data.get_data_google(tickers_instr, start, end)
    ...
    RawDataNZD =  quandl.get(tickers_instr_quandl[1], authtoken="PxdjCr_3nxsX_2JJd2si", start_date=start, rows=187)   
    RawDataBUND = quandl.get(tickers_instr_quandl[0], authtoken="PxdjCr_3nxsX_2JJd2si", start_date=start, rows=187)
    
    pr_instr = np.round(RawDataInstr.ix[slicekey], 2).tail(187)
    pr_instr['NZD'] = RawDataNZD
    # Reordering
    pr_instr = pr_instr[['SPY', 'FEZ', 'GLD', 'SLV', 'JJC', 'USO', 'UNG', 'FXE', 'FXB', 'FXA', 'FXY', 'FXF', 'FXC', 'NZD', 'UUP']].fillna(method='ffill')
    
    pr_factors = np.round(RawDataFactors.ix[slicekey], 2).tail(187)
    # Reordering
    pr_factors = pr_factors[['SPY', 'DBP', 'JJC', 'USO', 'UNG', 'UUP', 'FXY']]
    #pr_factors = pr_factors[['SPY', 'UUP', 'DBP', 'DBE']]
...
def get_day_stats(pr_instr=pr_instr, pr_factors=pr_factors, RawDataVol=RawDataVol):
    ...
    # Price plot
    pr_instr.tail(74).plot(subplots=True, title="INSTRUMENTS", layout=(5, 3), figsize=(15, 10), sharex=True) ;
    print('\v')
    ...
    ewma_vol_instr.tail(74).plot(subplots=True, title="VOLATILITY", layout=(5, 3), figsize=(15, 10), sharex=True) ;
...
def vol_model(RawDataVol=RawDataVol):
    ...
    RawDataVol['spx_ret'] = RawDataVol[[0]].pct_change()*100
    ...
    vol[['USO', 'CBOE/OVX - USO VIX (OVX)']].plot(title='OIL VOL RISK PREMIUM') ;
    vol[['GLD', 'CBOE/GVZ - GVZ']].plot(title='GOLD VOL RISK PREMIUM') ;
    vol[['SLV', 'CBOE/VXSLV - Close']].plot(title='SILVER VOL RISK PREMIUM') ;
    vol[['FXE', 'CBOE/EVZ - EVZ']].plot(title='EURO FX VOL RISK PREMIUM') ;
    print('\v')    
...
def get_weights(loc = 'local'):
    ...
    if loc == 'cloud':
    	...
    	weights = pd.read_csv('https://docs.google.com/spreadsheets/d/1bmy2DLu5NV5IP-mo9rGWOyHOx7bEfoglVZmzzuHi5zc/export?gid=0&format=xlsx', index_col=0)
    #print('\n', 'Weights\n', weights, '\n', '\n')
    return weights
...
def portf_risk():
    ...
    var = pd.DataFrame.ewm(rt_instr/100, span=32).var().dropna()['SPY']
    var = np.around(var, 5)
    cov = pd.DataFrame.ewm(rt_instr/100, span=32).cov().dropna()
    cov = cov.xs(key='SPY', axis=1).transpose()
    cov = np.around(cov, 5)
...
def risk_summary(plots = False, stress = 'none', ols = False):
    ...
    VaR_lim = VaR_single.loc[:, (VaR_single >= 50).any(axis=0)]
    ...
    print('Exp. Ret.=', "%.1f%%" % factMod_expRet, '   ', 'Std. Dev.=', "%.1f%%" % factMod_std, '   ', 'Vol.=', "%.0f%%" % factMod_vol, '   ', 'Id. Risk=', "%.1f%%" % fact_mod.mse_resid, '\n', '\n')
    if factMod_std > 10:
        print('*** ALERT: DAILY VOLATILITY EXCEEDING LIMIT, HEDGE NEEDED ***', '\n', '\n')
    print('Instruments Beta\n', '\n', beta_last,  '\n', '\n')
    print('Factors Beta\n', '\n', betaf.iloc[[-1]],  '\n', '\n')
    print('Portfolio Value at Risk\n', '\n', 'VaR (factors):', var_fact, '   ', 'VaR (hist.):', var_hist, '   ', 'CVaR (hist.):', cvar_hist,  '\n', '\n')
    if (var_fact or var_hist) > 500:
        print('*** ALERT: PORTFOLIO VaR TOO HIGH ***', '\n', '\n')
        
    # Plots
    if plots == True:
        beta.tail(100).plot(subplots=True, title="INSTRUMENTS BETAS", layout=(5, 3), figsize=(15, 10), sharex=True) ;
